			...
			<div class="row">
				...
				<div class="col-sm-9 padding-right">
					...
					if ($v1== "todos"){
								...
								$mysqli->set_charset("utf8");
							//MySqli Select Query
							$results = $mysqli->query("SELECT category, id FROM category ORDER BY category");
							...
							while($row = $results->fetch_assoc()) {
								...
								if ($mysqli2->connect_errno) {
									...
									$mysqli2->set_charset("utf8");
									//MySqli Select Query
									$results2 = $mysqli2->query("SELECT * FROM `list` WHERE `category` like ".$row["id"]);
									print '<div class="item-p"><ul>';
									while($row2 = $results2->fetch_assoc()) {
										print '<p><li><a href="servicio.php?id='.$row2["id"].'">'.$row2["name"].'</a><br></li></p>';
									}
									print '</ul></div>';
  
							} 
					}else{
					$mysqli3 = new mysqli("localhost", "espanoles", "coco_1drilo", "espenedin2");
										if ($mysqli3->connect_errno) {
											echo "Fallo al contenctar a MySQL: (" . $mysqli3->connect_errno . ") " . $mysqli3->connect_error;
										}
										$mysqli3->set_charset("utf8");
										//MySqli Select Query
										$results3 = $mysqli3->query("SELECT list.name, list.id as listid, category.id FROM list INNER JOIN category ON list.category=category.id where category.category like '".$v1."'");
										while($row3 = $results3->fetch_assoc()) {
										print '<li><div class="item-subtitle">'.$row3["name"].'       <a href="servicio.php?id='.$row3["listid"].'"><button type="button" class="btn btn-default get">Go</button></a></div></li>';
										print '<hr>';
										}  
					}					
					?>
